...
import pytest
from src.utils.load_env import REQUIRED_API_KEY_VARS, REQUIRED_VARS, load_dotenv_helper


def test_raises_on_missing_required_vars():
    empty_env = {var: "" for var in REQUIRED_VARS + REQUIRED_API_KEY_VARS}
...
def test_no_error_when_all_vars_present():
    ...
    full_env["OPENROUTER_API_KEY"] = "dummy_value"
    with patch("src.utils.load_env.load_dotenv", return_value=True), patch.dict("os.environ", full_env):
        load_dotenv_helper()


def test_no_error_with_openai_api_key_fallback():
    full_env = {var: "dummy_value" for var in REQUIRED_VARS}
    full_env["OPENAI_API_KEY"] = "dummy_value"
    with patch("src.utils.load_env.load_dotenv", return_value=True), patch.dict("os.environ", full_env, clear=True):
        load_dotenv_helper()
...
def test_all_required_vars_listed_in_error():
    ...
    assert "OPENROUTER_API_KEY or OPENAI_API_KEY" in error_message
