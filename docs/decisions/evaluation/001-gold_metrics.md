...
Initial metrics proposed for tracking and optimizing system against were:
- send rate
- edit distance
- session completion
- cost
- latency, 
- classifier-vs-drafter attribution

Half of these are component diagnostics, this doc cuts the list to three and explains what got dropped and why.

**NOTE** All thresholds in this doc are bootstrapped placeholders. They get refit to data once the eval dataset reaches ~50 labeled examples (see Threshold Calibration under Send Rate). We will use a couple hundred for regression gates in CI, and a larger set sampled from production once there's volume. Different sizes serve different jobs.
...
2. **User-outcome facing.** It scores what the user experiences (was the email sent, how long did it take, how much did it cost to run)
...
Component-level metrics to serve as diagnostics for gold metrics that will live in own ADR:
...
**Definition.** Of drafts produced by Inbox0 and surfaced to the user, the fraction that result in a sent email matching the draft within a reasonable window (default: 24h).
...
| Save Draft | User sends draft as-is, or with edits below edit-distance threshold | **Pass** |
| Save Draft | User sends draft with edits above edit-distance threshold | **Fail** |
...
Metrics considered, not shipped:
...
**Attribution**. Triage is currently fused into the drafter — the same LLM that writes a draft is implicitly deciding the email is worth drafting in the first place. Once the triage classifier ships, that decision becomes observable as a separate signal and the attribution columns in the send-rate rubric become computable.


### 2. Latency

**Definition.** Wall-clock time from email ingestion to draft appearing in Slack for user review.
...
| Send rate (Gmail outcome) | Gmail folder polling (Sent / Drafts) | Needed |
